...
export default function BreathingOverlay({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-opacity">
      <div
        className={`relative w-full max-w-3xl m-4 p-6 rounded-2xl shadow-2xl border border-white/20 bg-white/30 backdrop-blur-md transition-all duration-300 ${currentStep.color}`}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col md:flex-row items-center gap-4">
          <div>
            ...
            <select
              ...
              className="p-2 rounded border shadow bg-white/70 backdrop-blur-sm"
            >
